...
<div class="flex-1 p:2 sm:p-6 justify-between flex flex-col h-screen">
    <div class="flex sm:items-center justify-between py-3 border-b-2 border-gray-200">
       <div class="relative flex items-center space-x-4">
          ...
          <img src="https://images.unsplash.com/photo-1528797890300-0787fd964724?ixlib=rb-1.2.1&amp;ixid=eyJhcHBfaWQiOjEyMDd9&amp;auto=format&amp;fit=facearea&amp;facepad=3&amp;w=144&amp;h=144" alt="" class="w-10 sm:w-16 h-10 sm:h-16 rounded-full">
          </div>
       ...
       <div class="flex items-center space-x-2">
          ...
          {#if !address}
          <button on:click={signIn} class="inline-flex items-center justify-center rounded-lg border h-10 w-14 transition duration-500 ease-in-out text-gray-500 hover:bg-gray-300 focus:outline-none">
            Login
         </button>
         {:else}
            <button on:click={signOut} class="ml-4">
                Signout {address.substr(0, 3)}...{address.substr(-3)}
            </button>
         {/if}
       </div>
    </div>
